...
# sous problem
###
param I_re{ite in 1..nIte, bus in bus_i};
param I_im{ite in 1..nIte, bus in bus_i};
...
subject to ctr_v_re{bus in bus_i}: v_re[bus] = sum{ite in 1..nIte}V_re[bus]*lambda[ite];
subject to ctr_v_im{bus in bus_i}: v_im[bus] = sum{ite in 1..nIte}V_im[bus]*lambda[ite];

var i_re{bus in bus_i};
var i_im{bus in bus_i};

subject to ctr_i_re{bus in bus_i}: i_re[bus] = sum{ite in 1..nIte}I_re[bus]*lambda[ite];
subject to ctr_i_im{bus in bus_i}: i_im[bus] = sum{ite in 1..nIte}I_im[bus]*lambda[ite];

# (Sd*-S_gen*)/v_or* = (Sd*-S_gen*)/|v_or|² v_or
subject to i_balance_re{bus in bus_i}:
  +sum{ite in 1..nIte}I_re[bus]*lambda[ite]
...
subject to i_balance_im{bus in bus_i}:
  +sum{ite in 1..nIte}I_im[bus]*lambda[ite]
...
var I_or_re{ite in 1..nIte, (l,m) in branch};
var I_or_im{ite in 1..nIte, (l,m) in branch};

var I_ex_re{ite in 1..nIte, (l,m) in branch};
var I_ex_im{ite in 1..nIte, (l,m) in branch};
# s_or = v_or i_or*
# s_ex = v_ex i_ex*
var v_mod{bus in bus_i};
subject to ctr_v_mod{bus in bus_i}: v_mod[bus] = v_re[bus]*v_re[bus]+v_im[bus]*v_im[bus];

subject to ctr_i_re{bus in bus_i}: i_re[bus]*v_mod[bus] = 
  +(+Pd[bus]-(if bus in gen_i then p_gen[bus])/100)*v_re[bus]
  -(-Qd[bus]+(if bus in gen_i then q_gen[bus])/100)*v_im[bus]
  ;
subject to ctr_i_im{bus in bus_i}: i_im[bus]*v_mod[bus] = 
  +(+Pd[bus]-(if bus in gen_i then p_gen[bus])/100)*v_im[bus]
  +(-Qd[bus]+(if bus in gen_i then q_gen[bus])/100)*v_re[bus]
  ;
...
subject to ctr_v_min{bus in bus_i}: Vmin[bus]^2 <= v_mod[bus];
subject to ctr_v_max{bus in bus_i}: Vmax[bus]^2 >= v_mod[bus];
